# Author: Run Jin
# Perform network and pathway analysis on clustered performed 
...
results_dir_cemitool <- file.path(analysis_dir, "results", "CEMITool")
if(!dir.exists(results_dir_cemitool)){
  dir.create(results_dir_cemitool, recursive=TRUE)
}

plots_dir <- file.path(analysis_dir, "plots", "network")
...
for (i in 1:nrow(cc_data_match)){
  ...
  cg_of_interest <- cc_data_match[i,1]
  distance_param <- cc_data_match[i,2]
  cluster_param <- cc_data_match[i,3]
  cluster_n <- cc_data_match[i,4]
