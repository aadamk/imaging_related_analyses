---
title: Kaplan-Meier Survival Analyses for Medulloblastoma Samples per Gene Expression
  Level
author: "Run Jin"
date: "8/18/2021"
...
cohort_df <- histology_df %>% dplyr::filter(tumor_descriptor == "Initial CNS Tumor") %>%
  ...
  dplyr::filter(cancer_group %in% c("Medulloblastoma")) 
...
lr_survival_results_dir <- file.path(root_dir, "molecular_target_radiation", "results", "Medulloblastoma", "log_rank_survival")

km_survival_plots_dir <- file.path(root_dir, "molecular_target_radiation", "plots","Medulloblastoma", "KM_survival")
...
lr_os_result_dir <- file.path(lr_survival_results_dir, "OS")
if (!dir.exists(lr_os_result_dir )) {
  dir.create(lr_os_result_dir , recursive = TRUE)
}

readr::write_tsv(fit_slc7a5_df, file.path(lr_os_result_dir, "log_rank_survival_slc7a5.tsv"))
readr::write_tsv(fit_folh1_df, file.path(lr_os_result_dir, "log_rank_survival_folh1.tsv"))
```
...
km_survival_plot_os_dir <- file.path(km_survival_plots_dir, "OS")
if (!dir.exists(km_survival_plot_os_dir )) {
  dir.create(km_survival_plot_os_dir , recursive = TRUE)
}

cowplot::save_plot(filename = file.path(km_survival_plot_os_dir, "kaplan_meier_survival_slc7a5.png"), plot = surv_plot)
```
...
# We can save the plot like a normal ggplot
cowplot::save_plot(filename = file.path(km_survival_plot_os_dir, "kaplan_meier_survival_folh1.png"), plot = surv_plot)
```
...
# Save the table data in a TSV
lr_pfs_result_dir <- file.path(lr_survival_results_dir, "PFS")
if (!dir.exists(lr_pfs_result_dir )) {
  dir.create(lr_pfs_result_dir , recursive = TRUE)
}

readr::write_tsv(fit_slc7a5_pfs_df, file.path(lr_pfs_result_dir,"log_rank_survival_pfs_slc7a5.tsv"))
readr::write_tsv(fit_folh1_pfs_df, file.path(lr_pfs_result_dir,"log_rank_survival_pfs_folh1.tsv"))
```
...
# We can save the plot like a normal ggplot
km_survival_plot_pfs_dir <- file.path(km_survival_plots_dir, "PFS")
if (!dir.exists(km_survival_plot_pfs_dir )) {
  dir.create(km_survival_plot_pfs_dir , recursive = TRUE)
}

cowplot::save_plot(filename = file.path(km_survival_plot_pfs_dir, "kaplan_meier_survival_pfs_slc7a5.png"), plot = surv_plot_pfs)
```
...
# We can save the plot like a normal ggplot
cowplot::save_plot(filename = file.path(km_survival_plot_pfs_dir, "kaplan_meier_survival_pfs_folh1.png"), plot = surv_plot_pfs)
```
