---
title: "TPM Plots of LGG Samples per Harmonized Diagnosis"
author: "Run Jin"
date: "8/11/2021"
...
print(p)
ggsave("plots/slc7a5_harmonized_diagnosis_violin.png", p)
```
...
p<-combined %>%
  ...
  ggplot( aes(x=myaxis, y=FOLH1, fill=harmonized_diagnosis)) +
    ...
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
print(p)
ggsave("plots/folh1_harmonized_diagnosis_violin.png",p)
```
...
write_tsv(anova_result, "results/anova_result_slc7A5_hd.tsv")
```
...
write_tsv(tukey.test, "results/tukey_result_slc7A5_hd.tsv")
```
...
summary(res_aov)
anova_result <- summary(res_aov)[[1]] %>% rownames_to_column()
write_tsv(anova_result, "results/anova_result_folh1_hd.tsv")
```
...
tukey.test %>%
  ...
  kable_classic(full_width = F, html_font = "Cambria")
write_tsv(tukey.test, "results/tukey_result_folh1_hd.tsv")
```
Plotting the 95% interval
```{r}
plot(TukeyHSD(res_aov), las=1, cex.axis=0.4)
